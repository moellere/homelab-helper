...
- [ ] **Reconciler** (`engine/reconciler.py`) — the keystone; everything below leans on it.
  ...
  - [ ] `host.network.*` slice + NIC `PhysicalPart` / `Placement` lineage (apply lineage pattern to NICs; key on MAC; filter virtual interfaces; slot is the kernel interface name today, future probe will add PCI address)
